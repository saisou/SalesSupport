...
class StoreVisitDetailViewController: UITableViewController {
    ...
    @IBOutlet weak var openStatusBtn: UIButton!
    @IBOutlet var openStatusSelectionBtns: Array<UIButton>?
//    @IBOutlet weak var openStatus1: UIButton!
//    @IBOutlet weak var openStatus2: UIButton!
//    @IBOutlet weak var openStatus3: UIButton!
    ...
    var openStatusRBM = SSRadioButtonsController()
    ...
    let sliderItems = [101: ["未開店","開店３ヶ月未満","開店３ヶ月以降"],
                       102: ["未導入","3.24%未満","楽天SP","Coiney/Square","その他3.24%サービス","3.25%以上","不明・ヒアリングできず"],
                       103: ["ios所持","Android所持(iosなし）","未所持","不明・ヒアリングでいず"],
                       104: ["S","A","B","C","D","N","Z"],
                       105: ["現在利用中のもので満足","別サービスを導入したばかり","現金主義（カード需要なし）","IOSの準備ができない","決裁権がなく判断できない","その他","ヒアンリングできず"]
    ]
...
//            self.discussTimeDP.datePickerMode = .time
    ...
    func setDelegates() {
        ...
        openStatusRBM.delegate = self
        for btn in openStatusSelectionBtns! {
            openStatusRBM.addButton(btn)
        }
//        openStatusRBM.addButton(openStatus1)
//        openStatusRBM.addButton(openStatus2)
//        openStatusRBM.addButton(openStatus3)
    }
    ...
    func initStoreComponentValue() {
        ...
        if self.store?.opened_status != nil {
            ...
            self.openStatusBtn.setTitle(text, for: .normal)
            self.store?.opened_status = text
            for btn in openStatusSelectionBtns! {
                if btn.title(for: .normal) == text {
                    btn.isSelected = true
                    break
                }
            }
        } else {
            let text = self.sliderItems[101]![0]
            self.openStatusBtn.setTitle(text, for: .normal)
            self.store?.opened_status = text
            for btn in openStatusSelectionBtns! {
                if btn.title(for: .normal) == text {
    ...
    @IBAction func selectionBtnTap(_ sender: UIButton) {
        if self.hideOpenStatus && sender.tag == 100 {
            self.hideOpenStatus = false
            self.tableView.beginUpdates()
            self.tableView.endUpdates()
        } else if !self.hideOpenStatus && sender.tag == 100 {
            self.hideOpenStatus = true
            self.tableView.beginUpdates()
            self.tableView.endUpdates()
        } else if !self.hideOpenStatus && sender.tag == 101 {
            self.hideOpenStatus = true
            self.tableView.beginUpdates()
            self.tableView.endUpdates()
        }
...
//            self.openStatusLbl.text = textValue
    ...
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if hideDiscussDP && indexPath.section == 1 && indexPath.row == 1 {
            ...
            return 0
        } else if hideOpenStatus && indexPath.section == 0 && indexPath.row == 6 {
            return 0
        }
...
extension StoreVisitDetailViewController: SSRadioButtonControllerDelegate {
    func didSelectButton(selectedButton: UIButton?) {
        if selectedButton == nil {
            return
        }
        let text = selectedButton?.title(for: .normal)
...
extension StoreVisitDetailViewController: UITextFieldDelegate {
